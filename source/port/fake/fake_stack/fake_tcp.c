...
char pattern_CIPSNTPCFG_Query[] = "AT+CIPSNTPCFG?";
char pattern_CIPSNTPCFG_Cmd[] = "AT+CIPSNTPCFG=";

char pattern_CIPSNTPTIME_Query[] = "AT+CIPSNTPTIME?";
char pattern_CIPSNTPTIME_Resp[] = "+CIPSNTPTIME:Thu Aug 04 14:48:05 2016\r\n\r\nOK\r\n";
...
static bool fake_esp_sntp_timezone_setup(const char msg[])
{
	memset(txBuffer,0,sizeof(txBuffer));
	strcpy(txBuffer, pattern_OK);
	return true;
...
static bool fake_esp_sntp_timezone_request(const char msg[])
{
	memset(txBuffer,0,sizeof(txBuffer));
	return true;
}

static bool fake_esp_get_time(const char msg[])
{
	memset(txBuffer,0,sizeof(txBuffer));
	memcpy((void*)txBuffer, pattern_CIPSNTPTIME_Resp, strlen(pattern_CIPSNTPTIME_Resp));
	return true;
}
...
{  
	...
	if(strstr(msg, pattern_CIPSNTPCFG_Cmd))
    {
    	fake_esp_sntp_timezone_setup(msg);
        return true;
    }

	if(strstr(msg, pattern_CIPSNTPCFG_Query))
    {
    	fake_esp_sntp_timezone_request(msg);
        return true;
    }

	if(strstr(msg, pattern_CIPSNTPTIME_Query))
    {
    	fake_esp_get_time(msg);
        return true;
    }
